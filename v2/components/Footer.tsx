  ...
  return (
    ...
    >
      <div className="mx-auto max-w-6xl px-4 sm:px-6 lg:px-8">
            ...
            <ul className="space-y-2" role="list">
              {navLinks.map(({ href, label }) => (
                <motion.li key={href} variants={reducedMotion ? undefined : itemVariants} transition={t}>
                  <Link
                    href={href}
                    className="text-sm text-white/70 hover:text-white transition-colors focus:outline-none focus-visible:ring-2 focus-visible:ring-white/50 focus-visible:ring-offset-2 focus-visible:ring-offset-[var(--color-primary-dark)] py-0.5 w-fit block"
                  >
                    {label}
                  </Link>
                </motion.li>
            ...
            <ul className="space-y-2 text-sm" role="list">
              <motion.li variants={reducedMotion ? undefined : itemVariants} transition={t}>
                <Link
                  ...
                  className="text-white/70 hover:text-white transition-colors focus:outline-none focus-visible:ring-2 focus-visible:ring-white/50 focus-visible:ring-offset-2 focus-visible:ring-offset-[var(--color-primary-dark)] py-0.5 w-fit block"
                >
                  Privacy
                </Link>
              </motion.li>
        ...
        >
          <h3 className="text-xs font-semibold uppercase tracking-[0.2em] text-white/80 mb-3">
            Get mobility updates
          </h3>
          <p className="text-sm text-white/60 mb-4 max-w-md">
            Occasional news on Erasmus+ mobilities and opportunities in Málaga.
          </p>
          ...
          {newsletterMessage && (
            <p
              ...
              className={`mt-2 text-sm ${newsletterStatus === "error" ? "text-amber-400" : "text-white/80"}`}
              role="status"
            >
              {newsletterMessage}
            </p>
        ...
        <motion.div
          className="mt-12 pt-8 border-t border-white/10 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 text-sm text-white/50"
          initial={reducedMotion ? false : { opacity: 0 }}
          animate={show ? { opacity: 1 } : { opacity: 0 }}
